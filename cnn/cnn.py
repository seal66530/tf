...
def main(args):
    ...
    #max pool
    image_data_max_pool = tf.nn.max_pool(tf.expand_dims(image_data_rgb, 0),  #扩展维度到4
                                         ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')

    #image_data_max_pool = tf.to_float(image_data_max_pool)

    #avg pool
    image_data_avg_pool = tf.nn.avg_pool(tf.to_float(tf.expand_dims(image_data_rgb, 0)),  #扩展维度到4
                                         ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')
    ...
    with tf.Session() as sess:
        if args.action == "gray":
            ...
            print image_data_max_pool_np.shape
            ...
            plt.subplot(1,3,2)
            plt.imshow(image_data_max_pool_np[0,:,:,:])
            #np.savetxt("image_data_max_pool_np[0].txt",image_data_max_pool_np[0,:,:,0])
            #np.savetxt("image_data_max_pool_np[1].txt",image_data_max_pool_np[0,:,:,1])
            #np.savetxt("image_data_max_pool_np[2].txt",image_data_max_pool_np[0,:,:,2])
            #print image_data_max_pool_np

            #plt.subplot(1,3,3)
            #plt.imshow(image_data_avg_pool_np[0,:,:,:])
